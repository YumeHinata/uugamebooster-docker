...
pm_clean_network() {
    ...
    # clean switch vlan
    sections=$(uci show network | grep '=switch_vlan' | cut -d '=' -f 1 | grep -sE "network.vlan")
    for section in $sections; do
        uci del "$section"
    done
...
pm_build_network() {
    ...
    get_new_vid() {
        local new_vid=$(pm_config_get settings vlan_min_vid)
        [ -z "$new_vid" ] && new_vid="1"
    ...
    [ "1" != "$(uci -q get "$SERVICE".settings.ignore_vid_conflit)" ] && {
    ...
    if [ "portbase" = "$vlan_type" ]; then
        ...
        for port in $ports; do
            ...
            [ -z "$switch" ] || [ -z "$phy_id" ] || [ -z "$cpu_port" ] && {
                pm_config_set "$port" vid ""
                pm_config_set "$port" ifname "$base_iface"
                util_portmap_set "$port" "$base_iface"
                continue
            }
