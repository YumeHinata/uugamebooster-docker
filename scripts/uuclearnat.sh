#!/bin/sh
# ============================================================================
# uuclearnat — NAT learn/flush companion for uuplugin
# x86_64 native version (no QEMU, no /arm-root prefix)
#
...
#   3. Monitor mangle INPUT for client DROP rules (acceleration signal)
...
# Background reader: keeps read-end open so uuplugin writes don't block
(
    while true; do
...
# Background writer: holds write-end open so uuplugin reads don't EOF
(
...
# Give reader/writer a moment to open the FIFO
sleep 1
...
# uuplugin creates DROP rules on mangle INPUT (tcp dpt:53) per client IP.
# We use that as our client list signal.
SEEN=""
...
while true; do
    ...
    for IP in $CLIENTS; do
        ...
        # DNAT DNS → Google (uuplugin standard behavior)
