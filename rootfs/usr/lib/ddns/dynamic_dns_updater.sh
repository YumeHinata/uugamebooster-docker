...
uci -q set ddns.$SECTION_ID.lastreturn="nocfg"
...
uci -q set ddns.$SECTION_ID.lastreturn="nocfg"
uci -q commit ddns
write_log 14 "Service section disabled!"
}
[ -n "$service_name" ] && get_service_data update_url update_script UPD_ANSWER
[ -z "$update_url" -a -z "$update_script" ] && {
uci -q set ddns.$SECTION_ID.lastreturn="nocfg"
write_log 14 "No update_url found/defined or no update_script found/defined!"
}
[ -n "$update_script" -a ! -f "$update_script" ] && {
uci -q set ddns.$SECTION_ID.lastreturn="nocfg"
write_log 14 "Custom update_script not found!"
}
[ -z "$lookup_host" ] && {
...
while : ; do
get_local_ip LOCAL_IP $REGISTERED_IP
...
uci -q set ddns.$SECTION_ID.wanip="$LOCAL_IP"
uci -q commit ddns
...
errcode=$(cat "$DATFILE"|cut -d' ' -f1)
uci -q set ddns.$SECTION_ID.lastreturn="$errcode"
uci -q set ddns.$SECTION_ID.laststatus="error"
uci -q commit ddns
...
write_log 3 "Updating IP at DDNS provider failed, $LOCAL_IP != $REGISTERED_IP "
