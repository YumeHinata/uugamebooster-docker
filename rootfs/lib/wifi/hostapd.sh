	...
	*)
	...
	then
		...
		then
			...
			append "$var" "rsn_override_omit_rsnxe=1" "$N"
		fi
...
EOF
	...
	if [ $wps_vap_tie_dbdc -ne 0 ]; then
		...
		if [ "$force_hostapd_attach" -eq 1  ]; then
			...
			[ -n "$ifname" ] && run_hostapd_cli $ifname "/var/run/hostapd-$device" "close_log"
			if [ -n "$wps_possible" -a -n "$config_methods" ]; then
				pid=/var/run/hostapd_cli-$ifname.pid
				#run_hostapd_cli_daemon $ifname "/var/run/hostapd-$device" "-P $pid -a /lib/wifi/wps-hostapd-update-uci -B"
			...
			then
				...
				if [ "${type}" == "pbbs" ]; then
					...
					if [ "$map" -ge 3 -a $band -ne 3 ]; then
						run_hostapd_cli $ifname  "/var/run/hostapd-$device" "DPP_BOOTSTRAP_GEN $dpp_type $dpp_curve $dpp_key $supported_curves"
					else
						if [ $dpp_urihost_ifname ]; then
							run_hostapd_cli $ifname  "/var/run/hostapd-$device" "DPP_BOOTSTRAP_GEN $dpp_type $dpp_curve $channel mac=$(cat /sys/class/net/$ifname/address | sed 's/://g') $dpp_key host=$(ifconfig $dpp_urihost_ifname | grep "inet addr" | awk -F ':' '{print $2}' | awk '{print $1}') $supported_curves"
						else
							run_hostapd_cli $ifname  "/var/run/hostapd-$device" "DPP_BOOTSTRAP_GEN $dpp_type $dpp_curve  $channel mac=$(cat /sys/class/net/$ifname/address | sed 's/://g') $dpp_key $supported_curves"
						fi
					fi
				fi
				...
				if [ "$mdns_action" == "advertise" ]; then
					...
					if [ "$mdns_role" == "controller" ]; then
						res=$(hostapd_cli -i $ifname -p /var/run/hostapd-$device dpp_bootstrap_gen $dpp_type $dpp_curve)
						pkhash=echo $(hostapd_cli -i $ifname -p /var/run/hostapd-$device dpp_bootstrap_info $res) | sed -r 's/.*pkhash=(\S+).*/\1/'
				...
				if [ -z "$dpp_controller" ] && [ "$map" -eq 0 ]; then
					if [ "$dpp_auth_role" == "initiator" ]; then
						...
						then
							...
							run_hostapd_cli $ifname "/var/run/hostapd-$device" "dpp_configurator_add $dpp_curve"
						else
							run_hostapd_cli $ifname  "/var/run/hostapd-$device" "dpp_pkex_add own=1 init=1 $pkex_ver $pkex_identifier role=enrollee $pkex_code $tcp_addr"
						fi
					else
						...
						if [ "${type}" == "qrcode" ]; then
							...
							run_hostapd_cli $ifname  "/var/run/hostapd-$device" "dpp_pkex_add own=1 $pkex_ver $pkex_identifier role=enrollee $pkex_code"
						fi
